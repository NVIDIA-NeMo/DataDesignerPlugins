...
| `core_property_columns` | No | Supported string Word core property (`author`, `category`, `comments`, `content_status`, `identifier`, `keywords`, `language`, `last_modified_by`, `subject`, `title`, `version`) to Jinja2 template pairs. Unsupported names fail config validation. |
...
**Artifact publishing limitation.** Data Designer 0.9.1 through 0.9.3 do not register files
outside its parquet and processor output folders in `metadata.json`, and its
standard `push_to_hub()` upload handles only parquet, images, and processor
outputs. Consequently, the generated DOCX files are local artifacts: publishing
with `push_to_hub()` alone leaves `docx_path` references without matching files.
If publishing a dataset, upload `<output_subdir>/<name>/` separately at the same
relative path. Full automatic registration and upload require a Data Designer
binary-artifact API upstream.
...
string. Jinja templates also receive the unmodified mapping so values such as
`"true"` and `"null"` stay the same in document content and rendered metadata.
