...
def test_release_ref_for_package_requires_configured_template(tmp_path: Path) -> None:
    write_tap_pyproject(tmp_path, {"release-ref-template": None})
    config = load_tap_config(tmp_path)
...
def test_malformed_url_errors_deterministically(tmp_path: Path) -> None:
    write_tap_pyproject(tmp_path, {"package-index-url": "not-a-url"})

    with pytest.raises(TapConfigError) as exc_info:
        load_tap_config(tmp_path)

    message = str(exc_info.value)
    assert "[tool.ddp.tap].package-index-url" in message
