...
| `output_subdir` | No | Folder under the dataset directory. Defaults to `documents`. |
| `filename_template` | No | Jinja2 template for the file name. Defaults to `{{ doc_id }}.docx`. |
...
| `footer_template` | No | Jinja2 template for the page footer. |
...
under `processors-files/`. Data Designer reads every directory there back as a
